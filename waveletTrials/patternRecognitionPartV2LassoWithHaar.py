...
************************************************
************************************************
In this script, lasso models are trained with 
coefficients with level 3 haar tranformation
R^2 values converge to 0.41
Regresion is trained with 10 fold CV.
************************************************
************************************************
************************************************
"""
...
def regressor(X,y):
    ...
    reg = LassoLarsCV(cv=10,precompute=False).fit(X_train, y_train)
...
def wavelet(numberOfPep,dummymatrix):
    ...
    for noPeptide in range(0,numberOfPep):
       temp=[]
       coeffs=pywt.wavedec(dummymatrix[noPeptide][1][:],'haar',mode='symmetric',level=3)
       temp.append(coeffs[0][0])
       temp.append(coeffs[0][1])
       
       temp.append(coeffs[1][0])
       temp.append(coeffs[1][1])
       
       temp.append(coeffs[2][0])
       temp.append(coeffs[2][1])
       temp.append(coeffs[2][2])
       
       temp.append(coeffs[3][0])
       temp.append(coeffs[3][1])
       temp.append(coeffs[3][2])
       temp.append(coeffs[3][3])
       temp.append(coeffs[3][4])
...
#with open("C:/Users/"+userName+"/Desktop/BerkHoca/Results/0.3Lasso.csv", 'w', newline='') as myfile:
